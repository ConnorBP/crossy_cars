...
/// Shared 256px detail maps generated by `tools/build_pbr_details.py`.
/// Albedo multipliers decode as sRGB; vector/data maps stay linear. Every
/// handle is loaded once with a repeating sampler and reused by cached materials.
#[derive(Resource, Clone)]
pub(crate) struct PbrDetailAssets {
    pub(crate) concrete_albedo: Handle<Image>,
    pub(crate) foliage_albedo: Handle<Image>,
    pub(crate) traffic_paint_albedo: Handle<Image>,
    pub(crate) traffic_paint_orm: Handle<Image>,
    ...
    pub(crate) soil_orm: Handle<Image>,
}

impl FromWorld for PbrDetailAssets {
    fn from_world(world: &mut World) -> Self {
        let assets = world.resource::<AssetServer>();
        let load = |path: &'static str, is_srgb: bool| {
            assets
                .load_builder()
        ...
        let linear = |path| load(path, false);
        Self {
            concrete_albedo: load("textures/pbr_detail/concrete_albedo.png", true),
            foliage_albedo: load("textures/pbr_detail/foliage_albedo.png", true),
            traffic_paint_albedo: load("textures/pbr_detail/traffic_paint_albedo.png", true),
            traffic_paint_orm: linear("textures/pbr_detail/traffic_paint_orm.png"),
            plastic_normal: linear("textures/pbr_detail/plastic_normal.png"),
            plastic_orm: linear("textures/pbr_detail/plastic_orm.png"),
            concrete_normal: linear("textures/pbr_detail/concrete_normal.png"),
            concrete_orm: linear("textures/pbr_detail/concrete_orm.png"),
            wood_normal: linear("textures/pbr_detail/wood_normal.png"),
            wood_orm: linear("textures/pbr_detail/wood_orm.png"),
            grass_normal: linear("textures/pbr_detail/grass_normal.png"),
            grass_orm: linear("textures/pbr_detail/grass_orm.png"),
            soil_orm: linear("textures/pbr_detail/soil_orm.png"),
        }
    }
}
...
impl Plugin for TexturesPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<PbrDetailAssets>()
...
impl FromWorld for TextureAssets {
    fn from_world(world: &mut World) -> Self {
        let details = world.get_resource::<PbrDetailAssets>().cloned();
        world.resource_scope::<Assets<Image>, _>(|world, mut images| {
            let (
                ...
                let grass_normal = details.as_ref().map_or_else(
                    ...
                    |assets| assets.grass_normal.clone(),
                );
                // Minimal ECS harnesses have no AssetServer and retain the old
                // procedural relief; production uses the optimized real map.
                let sidewalk_normal = details.as_ref().map_or_else(
                    || images.add(concrete_normal_map()),
                    |assets| assets.concrete_normal.clone(),
                );
                ...
                let grass = materials.add(toy_material(
                    ...
                    StandardMaterial {
                        ...
                        metallic_roughness_texture: details
                            .as_ref()
                            .map(|assets| assets.grass_orm.clone()),
                        occlusion_texture: details.as_ref().map(|assets| assets.grass_orm.clone()),
                ...
                let sidewalk = materials.add(toy_material(
                    ...
                    StandardMaterial {
                        ...
                        metallic_roughness_texture: details
                            .as_ref()
                            .map(|assets| assets.concrete_orm.clone()),
                        occlusion_texture: details
                            .as_ref()
                            .map(|assets| assets.concrete_orm.clone()),
                ...
                let park_ground = std::array::from_fn(|variant| {
                    materials.add(toy_material(
                        ...
                        StandardMaterial {
                            ...
                            normal_map_texture: Some(details.as_ref().map_or_else(
                                || images.add(park_ground_normal_map(variant)),
                                |assets| assets.grass_normal.clone(),
                            )),
                            metallic_roughness_texture: details
                                .as_ref()
                ...
                let orchard_ground = std::array::from_fn(|variant| {
                    materials.add(toy_material(
                        ...
                        StandardMaterial {
                            ...
                            metallic_roughness_texture: details
                                .as_ref()
                                .map(|assets| assets.soil_orm.clone()),
                            occlusion_texture: details
                                .as_ref()
                                .map(|assets| assets.soil_orm.clone()),
                            uv_transform: Affine2::from_scale(Vec2::splat(8.0)),
...
fn orchard_ground_pixel(variant: usize, x: u32, y: u32) -> [u8; 4] {
    ...
    let soil = (tileable_value_noise(x, y, 12, 11 + variant as u32) * 7.0).round() as i32;
    let soil_fine = (tileable_value_noise(x, y, 6, 19 + variant as u32) * 3.0).round() as i32;
    let grain = signed_noise2(x + 29, y + variant as u32 * 43) * 3 / 128;
    let mulch = if noise(x + variant as u32 * 17, y) & 0x3f == 0 {
        -10
    } else {
        0
    };
    ...
    [
        clamp_byte(b[0] + soil + soil_fine + grain + mulch + leaf[0]),
        clamp_byte(b[1] + soil + soil_fine + grain + mulch + leaf[1]),
        clamp_byte(b[2] + soil / 2 + soil_fine / 2 + grain + mulch / 2 + leaf[2]),
        255,
    ]
}
...
fn orchard_ground_normal_map(variant: usize) -> Image {
    make_normal_map(
        move |x, y| {
            tileable_value_noise(x, y, 12, 41 + variant as u32) * 0.45
                + tileable_value_noise(x, y, 6, 47 + variant as u32) * 0.18
                + signed_noise2(x * 2 + variant as u32 * 13, y * 2) as f32 / 128.0 * 0.06
        },
        0.26,
    )
}
...
mod tests {
    ...
    #[test]
    fn orchard_detail_has_no_directional_row_energy() {
        fn directional_energy(image: &Image) -> (u64, u64) {
            let pixels = pixels(image);
            let mut horizontal = 0u64;
            let mut vertical = 0u64;
            for y in 0..TEX_SIZE as usize {
                for x in 0..TEX_SIZE as usize {
                    let here = y * TEX_SIZE as usize * 4 + x * 4;
                    let right = y * TEX_SIZE as usize * 4 + ((x + 1) % TEX_SIZE as usize) * 4;
                    let down = ((y + 1) % TEX_SIZE as usize) * TEX_SIZE as usize * 4 + x * 4;
                    let lum = |index: usize| {
                        (u32::from(pixels[index]) * 54
                            + u32::from(pixels[index + 1]) * 183
                            + u32::from(pixels[index + 2]) * 19)
                            / 256
                    };
                    horizontal += u64::from(lum(here).abs_diff(lum(right)));
                    vertical += u64::from(lum(here).abs_diff(lum(down)));
                }
            }
            (horizontal, vertical)
        }
        ...
        for variant in 0..GROUND_VARIANTS {
            for image in [
                ...
                let (horizontal, vertical) = directional_energy(&image);
                let larger = horizontal.max(vertical);
                let smaller = horizontal.min(vertical).max(1);
                assert!(
                    larger * 100 <= smaller * 135,
                    "orchard detail must stay isotropic: h={horizontal}, v={vertical}"
                );
            }
        }
    }
