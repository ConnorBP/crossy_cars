// Pure TypeScript mirror of the immutable roady-rules.v2 protocol.
//
...
export const CLUCK_HUNT_CATEGORY = "rotation.v1.cluck_hunt";
export const RIGHT_OF_WAY_CATEGORY = "rotation.v1.right_of_way";
...
export const ROTATION_DOMAIN = "roady.rotation.v2.rotation";
export const SCHEDULED_EVENTS_DOMAIN = "roady.rotation.v2.scheduled_events";
export const FRENZY_INTERVAL_DOMAIN = "roady.rotation.v2.frenzy.interval";
export const FRENZY_ROLL_DOMAIN = "roady.rotation.v2.frenzy.roll";
export const FRENZY_KIND_DOMAIN = "roady.rotation.v2.frenzy.kind";
export const FRENZY_POSITION_DOMAIN = "roady.rotation.v2.frenzy.position";
export const FRENZY_RELOCATION_DOMAIN = "roady.rotation.v2.frenzy.relocation";
...
export function terminalBytes(value: ConductTerminal): Uint8Array {
  if (ENCODER.encode(value.build).length > MAX_BUILD_BYTES) throw new RangeError("build is too long");
  const writer = new CanonicalWriter();
...
export function eventRecord(event: ProtocolEvent): Uint8Array {
  const writer = new CanonicalWriter();
  writer.lp1("roady.v2.event"); writer.u32(event.seq); writer.u64(event.activeMs);
...
export function evidenceBytes(sessionId: string, eventCount: bigint, storedLedger: Uint8Array): Uint8Array {
  ...
  const writer = new CanonicalWriter();
  writer.lp1("roady.v2.evidence"); writer.lp1(sessionId); writer.u32(eventCount); writer.lp4(storedLedger);
  const bytes = writer.bytes();
...
export async function finalRoot(
  ...
  const writer = new CanonicalWriter();
  writer.lp1("roady.v2.root"); writer.raw32(h0); writer.raw32(hN); writer.raw(terminalBytes(terminal));
  return sha256(writer.bytes());
}
...
export function scoreHmacInput(input: ScoreHmacInput): Uint8Array {
  const writer = new CanonicalWriter();
  writer.lp1("roady.v2.score"); writer.u8(PROTOCOL_VERSION); writer.u8(RULES_VERSION);
  writer.u8(POLICY_VERSION); writer.lp1(MODE); writer.lp1(input.category);
